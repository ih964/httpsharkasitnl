...
export default function InvoiceMakerForceLeftRight() {
  ...
  const setParty = (type: "sender" | "customer", field: keyof Party, value: string) => {
    if (type === "sender") setSender((old) => ({ ...old, [field]: value }));
  ...
  const downloadPdf = () => {
    ...
    if (sidebar) {
      ...
      doc.setFont("helvetica", "normal"); doc.setFontSize(8); [invoiceNumber, sender.company, sender.email, sender.phone, sender.kvk ? `KvK ${sender.kvk}` : ""].filter(Boolean).forEach((text, index) => doc.text(String(text), 10, 38 + index * 8));
      doc.setTextColor(17, 24, 39); y = 24;
    } else {
      const dark = template === "premium" ? [12, 18, 34] : template === "service" ? [7, 89, 133] : template === "split" ? [30, 64, 175] : [17, 24, 39];
      if (["classic", "minimal", "letterhead", "compact"].includes(template)) {
        doc.setTextColor(17, 24, 39); doc.setFont("helvetica", template === "minimal" ? "normal" : "bold"); doc.setFontSize(template === "minimal" ? 30 : 22); doc.text(template === "minimal" ? "Invoice" : "FACTUUR", 16, 24); doc.line(16, 32, w - 16, 32); y = 46;
      } else {
        doc.setFillColor(dark[0], dark[1], dark[2]); doc.rect(0, 0, w, 42, "F");
        if (template === "premium") { doc.setFillColor(181, 143, 74); doc.rect(0, 40, w, 2, "F"); }
        doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(24); doc.text(template === "service" ? "Werkzaamheden & factuur" : "Factuur", 16, 24);
        doc.setFontSize(9); doc.text(`${invoiceNumber} · ${templateNames[template]}`, 16, 34); y = 56; doc.setTextColor(17, 24, 39);
      }
    }
    ...
    doc.rect(left, y, tableWidth, 8, "F"); doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(8);
    doc.text(template === "service" ? "Werkzaamheden" : "Omschrijving", left + 2, y + 5.5); doc.text(template === "service" ? "Uren" : "Aantal", left + tableWidth - 82, y + 5.5, { align: "right" }); doc.text(template === "service" ? "Tarief" : "Prijs", left + tableWidth - 56, y + 5.5, { align: "right" }); doc.text("Totaal", left + tableWidth - 2, y + 5.5, { align: "right" }); y += 11;
    doc.setTextColor(17, 24, 39); doc.setFont("helvetica", "normal");
    lines.filter((line) => line.description.trim()).forEach((line) => { const subtotal = line.quantity * line.price; const total = subtotal + subtotal * line.vat / 100; const description = doc.splitTextToSize(line.description, tableWidth - 88); const rowHeight = Math.max(template === "compact" ? 6 : 9, description.length * 4 + 4); doc.setDrawColor(229, 231, 235); doc.line(left, y - 3, left + tableWidth, y - 3); doc.text(description, left + 2, y + 2); doc.text(String(line.quantity), left + tableWidth - 82, y + 2, { align: "right" }); doc.text(pdfMoney(line.price), left + tableWidth - 56, y + 2, { align: "right" }); doc.text(pdfMoney(total), left + tableWidth - 2, y + 2, { align: "right" }); y += rowHeight; });
    y += 8; const totalX = w - 92; doc.text("Subtotaal", totalX, y); doc.text(pdfMoney(totals.subtotal), w - 16, y, { align: "right" }); y += 7; doc.text("BTW", totalX, y); doc.text(pdfMoney(totals.vat), w - 16, y, { align: "right" }); y += 5; doc.line(totalX, y, w - 16, y); y += 9; doc.setFont("helvetica", "bold"); doc.setFontSize(13); doc.text("Totaal EUR", totalX, y); doc.text(pdfMoney(totals.total), w - 16, y, { align: "right" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <div><h1 className="text-3xl font-bold">Factuur Maker</h1><p className="text-muted-foreground">Links invullen, rechts live preview.</p></div>
      ...
      <div className="overflow-x-auto pb-4">
        <div className="min-w-[1180px] grid grid-cols-[430px_minmax(0,1fr)] gap-6 items-start">
          <div className="space-y-6 max-h-[calc(100vh-130px)] overflow-y-auto pr-2">
            ...
            <PartyCard title="Afzender" party={sender} onChange={(field, value) => setParty("sender", field, value)} />
            <PartyCard title="Klant" party={customer} onChange={(field, value) => setParty("customer", field, value)} />
