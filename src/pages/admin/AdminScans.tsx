...
export default function AdminScans() {
  ...
  const load = async () => {
    ...
    const { data, error } = await client
      ...
      .select("id,company_name,contact_name,email,phone,employee_count,status,consent_marketing,created_at,assessment_runs(id,total_score,risk_level,category_scores,created_at)")
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((lead) => [lead.company_name, lead.contact_name, lead.email, lead.phone ?? ""].some((value) => value.toLowerCase().includes(query)));
  }, [leads, search]);
