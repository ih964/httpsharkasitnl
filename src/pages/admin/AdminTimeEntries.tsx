...
const AdminTimeEntries = () => {
  ...
  const loadData = async () => {
    ...
    const [e, c] = await Promise.all([
      supabase.from("time_entries").select("*, invoices(invoice_number)").order("work_date", { ascending: false }),
  ...
  }, [entries, filterCustomer, filterMonth, filterYear, filterStatus, search]);
  ...
  // Totals on selection or filtered
  ...
  const createInvoiceFromSelection = async () => {
    ...
    if (!customerId) {
      toast({ title: "Geselecteerde uren hebben geen klant", variant: "destructive" });
      return;
    }
    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <Input className="pl-8" placeholder="Zoek omschrijving..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={selectAllFiltered}>Selecteer alles binnen filter</Button>
          {selected.size > 0 && (
            <Button variant="ghost" size="sm" onClick={clearSelection}>Selectie wissen ({selected.size})</Button>
          )}
        </div>
        <Button
        ...
        >
          ...
          {creatingInvoice ? "Bezig..." : `Factuur maken van selectie (${selected.size})`}
        </Button>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={10} className="text-center py-8 text-muted-foreground">Geen urenregels gevonden</TableCell></TableRow>
              ) : (
                filtered.map((e) => {
