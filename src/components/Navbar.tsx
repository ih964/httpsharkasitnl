...
const serviceLinks = [
  { name: "IT Beheer & Support", href: "/diensten/support" },
  { name: "Microsoft 365", href: "/diensten/support" },
  { name: "Werkplekbeheer", href: "/diensten/support" },
  { name: "Remote Support", href: "/diensten/support" },
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden xl:flex items-center gap-5">
              <div className="relative group">
                <button className="inline-flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors py-3">
                  Diensten
                ...
                <div className="invisible opacity-0 translate-y-2 group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 absolute left-0 top-full min-w-[260px] rounded-2xl border border-border/50 bg-background/95 backdrop-blur-xl shadow-2xl p-3">
                  {serviceLinks.map((link) => (
                    <a key={link.name} href={link.href} className="block rounded-xl px-4 py-3 text-sm text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors">
                      {link.name}
                    </a>
                  ))}
                </div>
        ...
        <div className="xl:hidden glass border-b border-border/30 bg-background/95 backdrop-blur-xl max-h-[calc(100vh-80px)] overflow-y-auto">
          <div className="container px-6 py-6 space-y-6">
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-wider text-primary">Diensten</p>
              {serviceLinks.map((link) => (
                <a key={link.name} href={link.href} className="block rounded-xl px-4 py-3 text-base font-medium text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors" onClick={() => setIsOpen(false)}>
                  {link.name}
                </a>
              ))}
            </div>
            ...
            <Button variant="hero" className="w-full" asChild>
              <a href="/#contact" onClick={() => setIsOpen(false)}>Plan gratis IT-check</a>
            </Button>
          </div>
        </div>
      )}
